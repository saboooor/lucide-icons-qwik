...
const globalProps = ['Infinity'];

// remove duplicate icons and fix some icon names to avoid shadowing of global properties
const icons = Object.keys(lucideIcons).reduce(
  (acc: Record<string, IconNode>, name) => {
    const icon = lucideIcons[name as keyof typeof lucideIcons];
    const duplicate = Object.values(acc).find(
      (i) => JSON.stringify(i) === JSON.stringify(icon)
    );
...
function buildContent(icon: IconNode) {
  const elements = icon.map((t) => {
    const tag = t[0];
    const attrs = Object.entries(t[1])
      .map((v) => `${v[0]}="${v[1]}"`)
      .join(' ');
    return `<${tag} ${attrs}/>`;
  });
...
function buildIcon() {
  ...
  const template = readFileSync('./icon.tsx.template', 'utf8').replace(
    '{{BASE_ICON_REL_PATH}}',
    baseIconPath
  );
